...
from src.model.wrapper import get_model # DuoduoCLIP, PYTHOPATH must have the path to this folder
...
class MaskCLIPFeaturizer(nn.Module):
    def forward(self, img):
        raise NotImplementedError("Should implement the forward method")
    
    @staticmethod
    def from_name(clip_model_name):
        if clip_model_name == 'ViT-L/14@336px':
            return StandardMaskCLIPFeaturizer(clip_model_name)
        elif clip_model_name == 'Four_1to6F_bs1600_LT6.ckpt':
            return DuoduoCLIPFeaturizer(clip_model_name)
        else:
            raise ValueError(f"Unknown CLIP model name: {clip_model_name}")
    
class StandardMaskCLIPFeaturizer(MaskCLIPFeaturizer):
...
class DuoduoCLIPFeaturizer(MaskCLIPFeaturizer):
    def __init__(self, clip_model_name):
        super().__init__()
        self.model = get_model(clip_model_name)
        self.model.eval()
        self.patch_size = self.model.duoduoclip.visual.patch_size

    def forward(self, img):
        b, input_size_h, input_size_w, _ = img.shape
        patch_h = input_size_h // self.patch_size
        patch_w = input_size_w // self.patch_size
        features = self.model.get_patch_encodings(img).to(torch.float32)
        return features.reshape(b, patch_h, patch_w, -1).permute(0, 3, 1, 2)
...
def batch_extract_feature(image_paths: List[str], args):
    ...
    clip_model = MaskCLIPFeaturizer.from_name(args.clip_model_name).cuda().eval()
    ...
    for i in trange(len(image_paths)):
        ...
        if isinstance(clip_model, StandardMaskCLIPFeaturizer):
            raw_input_image = raw_transform(Image.fromarray(image))
        elif isinstance(clip_model, DuoduoCLIPFeaturizer):
            raw_input_image = torch.from_numpy(image)
